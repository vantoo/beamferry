...
assert.ok(sender.includes('new URL("desktop-receiver/", RECEIVER_URL).href'));
...
assert.ok(template.includes('name="beamferry-receiver-url" content=""'));
assert.ok(sender.includes("function resolveReceiverUrl"));
assert.ok(sender.includes("embedded || hosted || DEFAULT_RECEIVER_URL"));
assert.ok(sender.includes("function withEmbeddedReceiverUrl"));
assert.ok(sender.includes('fetch(sourceUrl, { cache: "no-store" })'));
assert.ok(sender.includes("new Blob([withEmbeddedReceiverUrl(html)]"));
assert.ok(sender.includes('downloadSender.addEventListener("click", downloadOfflineSender)'));
